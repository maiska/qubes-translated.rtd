...
To avoid this risk, use a [USB qube](/es/doc/usb-qubes/).
...
Because of this, the benefits of using a [USB qube](/es/doc/usb-qubes/) entrusted with a keyboard or other interface device are much smaller than using a fully untrusted USB qube.
...
One way to achieve this is to use a [YubiKey](/es/doc/YubiKey/), or some other hardware token, or even to manually enter a one-time password.
