...
lang: es
layout: doc
permalink: /es/doc/standalone-and-hvm/
redirect_from:
- /es/doc/hvm/
- /es/wiki/HvmCreate/
- /es/doc/HvmCreate/
...
A [StandaloneVM](/es/doc/glossary/#standalonevm) is a type of VM in Qubes that is created by cloning a [TemplateVM](/es/doc/templates/).
...
Meanwhile, a [Hardware-assisted Virtual Machine (HVM)](/es/doc/glossary/#hvm), also known as a "Fully-Virtualized Virtual Machine," utilizes the virtualization extensions of the host CPU.
These are typically contrasted with [Paravirtualized (PV)](/es/doc/glossary/#pv) VMs.
...
By default, every Qubes VM runs in [PVH](/es/doc/glossary/#pvhvm) mode (which has security advantages over both PV and HVM) except for those with attached PCI devices, which run in HVM mode.
...
In fact, this is so common that [StandaloneHVMs](/es/doc/glossary/#standalonehvm) are typically just called "HVMs."
...
Normally Qubes agent scripts (or services on Windows) running within each AppVM are responsible for setting up networking within the VM according to the configuration created by Qubes (through [keys](/es/doc/vm-interface/#qubesdb) exposed by dom0 to the VM).
...
There is [opt-in support](/es/doc/networking/#ipv6) for IPv6 forwarding.

## Using TemplateBasedHVMs
<a id="using-templatebasedhvms"></a>

Qubes allows HVMs to share a common root filesystem from a select TemplateVM (see [TemplateHVM](/es/doc/glossary/#templatehvm) and [TemplateBasedHVM](/es/doc/glossary/#templatebasedhvm)).
...
Set memory as appropriate, and install the OS into this template in the same way you would install it into a normal HVM -- please see instructions on [this page](/es/doc/hvm-create/).
...
Please see [this page](/es/doc/windows-appvms/) for specific advice on installing and using Windows-based Templates.
...
HVM domains (including Windows VMs) can be [assigned PCI devices](/es/doc/assigning-devices/) just like normal AppVMs.
...
- [Windows VMs](/es/doc/windows-vm/)
- [LinuxHVMTips](/es/doc/linux-hvm-tips/)