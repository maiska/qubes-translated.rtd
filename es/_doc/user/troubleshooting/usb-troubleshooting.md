...
For guidance setting up a USB qube, see the [USB documentation](/es/doc/usb-devices/#creating-and-using-a-usb-qube).
...
If this isn't an option, you can manually recover from the situation by following the instructions at the [Block Devices documentation](/es/doc/block-devices/#what-if-i-removed-the-device-before-detaching-it-from-the-vm)
...
When trying to [create and use a USB qube](/es/doc/usb-devices/#creating-and-using-a-usb-qube) with the `qubes-usb-proxy` package, you may receive this error: `ERROR: qubes-usb-proxy not installed in the VM`.
