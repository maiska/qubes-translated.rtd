...
lang: es
layout: doc
permalink: /es/doc/usb-troubleshooting/
ref: 234
...
For more details on this issue along with possible solutions, look at [PCI passthrough issues](/es/doc/pci-troubleshooting/#pci-passthrough-issues).
...
For guidance setting up a USB qube, see the [USB documentation](/es/doc/how-to-use-usb-devices/#creating-and-using-a-usb-qube).
...
If this isn't an option, you can manually recover from the situation by following the instructions at the [Block Devices documentation](/es/doc/how-to-use-block-storage-devices/#what-if-i-removed-the-device-before-detaching-it-from-the-vm)
...
Please review the notes in the `qvm-prefs` man page and [here](/es/doc/how-to-use-devices/) and be aware of the potential risks.
...
To solve the problem, disable the USB qube by not having it autostart, or unassigning your USB controller(s) from it. If you had created the USB qube by checking the box in the installer, then your USB controller(s) are probably hidden from dom0. To unhide them, reverse the procedure described in the [USB Qubes documentation](/es/doc/usb-qubes/#how-to-hide-all-usb-controllers-from-dom0) (under "How to hide all USB controllers from dom0"). That is, remove `rd.qubes.hide_all_usb`, instead of adding it.
...
When trying to [create and use a USB qube](/es/doc/how-to-use-usb-devices/#creating-and-using-a-usb-qube) with the `qubes-usb-proxy` package, you may receive this error: `ERROR: qubes-usb-proxy not installed in the VM`.
