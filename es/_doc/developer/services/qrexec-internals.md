...
For the implementation of qrexec2, see [here](/es/doc/qrexec2/#qubes-rpc-internals).*)
...
`qrexec-policy` is a mechanism for evaluating whether an RPC call should be allowed. For introduction, see [Qubes RPC administration](/es/doc/qrexec/#qubes-rpc-administration).
...
It is a [socket-based Qubes RPC service](/es/doc/qrexec-socket-services/). Requests are in JSON format, and response is simple ASCII.
