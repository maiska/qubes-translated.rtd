...
Once you've built the development AppVM, open a Terminal window to it and install the necessary dependencies (see [QubesBuilder](/es/doc/qubes-builder/) for more info):
...
See [Verifying Signatures](/es/security/verifying-signatures/#1-get-the-qubes-master-signing-key-and-verify-its-authenticity) for verification sources.
...
Let's configure the builder first (see [procedure](/es/doc/qubes-iso-building/#build-using-manual-steps) at bottom if you would prefer to manually configure):
...
See the related note under [Extra Whonix Build Options](/es/doc/building-whonix-template/).

You may also want to add `COMPONENTS := $(filter-out gcc,$(COMPONENTS))` to bypass a multiple hour compile step.
See [QubesBuilder](/es/doc/qubes-builder/#use-pre-built-qubes-packages) for more detail.
