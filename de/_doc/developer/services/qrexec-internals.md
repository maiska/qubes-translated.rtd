...
`qrexec-policy` is a mechanism for evaluating whether an RPC call should be allowed. For introduction, see [Qubes RPC administration](/de/doc/qrexec/#qubes-rpc-administration).
...
It is a [socket-based Qubes RPC service](/de/doc/qrexec-socket-services/). Requests are in JSON format, and response is simple ASCII.
