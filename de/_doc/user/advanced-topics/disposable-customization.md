...
lang: de
layout: doc
permalink: /de/doc/disposable-customization/
redirect_from:
- /de/doc/DispVMCustomization/
- /de/doc/UserDoc/DispVMCustomization/
- /de/doc/disposablevm-customization/
...
A [disposable](/de/doc/disposable/) can be based on any [app qube](/de/doc/glossary/#app-qube).
You can also choose to use different [disposable templates](/de/doc/glossary/#disposable-template) for different disposables.
...
If you wish to use a [Minimal Template](/de/doc/templates/minimal/) as a disposable template, please see the [Minimal Template](/de/doc/templates/minimal/) page.
...
You can use a [named disposable](/de/doc/glossary/#named-disposable) for `sys-*` as long as it is stateless.
...
**Note** You can use `qvm-pci` to [determine](/de/doc/how-to-use-pci-devices/#qvm-pci-usage) the `<BDF>`.
Also, you will often need to include the `-o no-strict-reset=True` [option](/de/doc/how-to-use-pci-devices/#no-strict-reset) with USB controllers.
...
6. Attach the network PCI device(s) to `disp-sys-net` (finding and assigning PCI devices can be found [here](/de/doc/how-to-use-pci-devices/):
...
5. Attach the USB controller to the `disp-sys-usb`:
   >_**Note:**_ Most of the commonly used USB controllers (all Intel integrated controllers) require the `-o no-strict-reset=True` option to be set. Instructions detailing how this option is set can be found [here](/de/doc/how-to-use-pci-devices/#no-strict-reset).
...
If the `disp-sys-usb` does not start, it could be due to a PCI passthrough problem. For more details on this issue along with possible solutions, users can look [here](/de/doc/pci-troubleshooting/#pci-passthrough-issues).
