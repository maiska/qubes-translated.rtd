...
Selecting "Yes to All" will add a line in the corresponding [RPC Policy](/de/doc/rpc-policy/) file.
...
* `vault`
   ...
   No files or text is *ever* [copied](/de/doc/how-to-copy-and-move-files/#security) or [pasted](/de/doc/how-to-copy-and-paste-text/#security) into this VM under *any* circumstances.

* `work-gpg`

   This is a network-isolated VM.
   This VM is used *only* as the GPG backend for `work-email`.
   The secret subkeys (but *not* the master secret key) are [copied](/de/doc/how-to-copy-and-move-files/#security) from the `vault` VM to this VM.
   Files from less trusted VMs are *never* [copied](/de/doc/how-to-copy-and-move-files/#security) into this VM under *any* circumstances.
...
<sup>\*</sup>In order to gain access to the `vault` VM, the attacker would require the use of, e.g., a general Xen VM escape exploit or a [signed, compromised package which is already installed in the template](/de/doc/templates/#trusting-your-templates) upon which the `vault` VM is based.
