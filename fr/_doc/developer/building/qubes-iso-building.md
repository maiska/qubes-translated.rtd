...
lang: fr
layout: doc
permalink: /fr/doc/qubes-iso-building/
redirect_from:
- /fr/doc/QubesR3Building/
- /fr/doc/qubes-r3-building/
- /fr/wiki/QubesR3Building/
ref: 63
...
Once you've built the development AppVM, open a Terminal window to it and install the necessary dependencies (see [QubesBuilder](/fr/doc/qubes-builder/) for more info):
...
See [Verifying Signatures](/fr/security/verifying-signatures/#1-get-the-qubes-master-signing-key-and-verify-its-authenticity) for verification sources.
...
-----------------
<a id="build-using-setup-script"></a>

Let's configure the builder first (see [procedure](/fr/doc/qubes-iso-building/#build-using-manual-steps) at bottom if you would prefer to manually configure):
...
See the related note under [Extra Whonix Build Options](/fr/doc/building-whonix-template/).

You may also want to add `COMPONENTS := $(filter-out gcc,$(COMPONENTS))` to bypass a multiple hour compile step.
See [QubesBuilder](/fr/doc/qubes-builder/#use-pre-built-qubes-packages) for more detail.
...
-----------------
<a id="build-using-manual-steps"></a>
