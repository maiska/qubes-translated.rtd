...
documentation](/fr/doc/#external-documentation) (see
...
send it to the appropriate [mailing list](/fr/support/). If you see that something
...
guidelines](/fr/doc/issue-tracking/). (If you report an issue with the
...
* We don't want you to spend time and effort on a contribution that we can't
  ...
  issue](/fr/doc/issue-tracking/) for it first so that we can make sure we're on
...
table of contents)](/fr/doc/) --- for example, if you're adding a new page,
...
The source file for the [documentation index (a.k.a. table of contents)](/fr/doc/)
lives here:
...
[TemplateVM](/fr/doc/templates/). However, it would not make sense for a
...
* Familiarize yourself with the terms defined in the
  [glossary](/fr/doc/glossary/). Use these terms consistently and accurately
...
* In order to enable offline browsing and automatic onion redirection, always
  ...
  exceptions:
  * The signed plain text portions of [QSBs](/fr/security/qsb/) and
    [Canaries](/fr/security/canary/)
...
coding style guidelines](/fr/doc/coding-style/#commit-message-guidelines).