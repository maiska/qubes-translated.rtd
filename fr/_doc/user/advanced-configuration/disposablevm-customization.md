...
lang: fr
layout: doc
permalink: /fr/doc/disposablevm-customization/
redirect_from:
...
A [DisposableVM](/doc/disposablevm) can be based on any [TemplateBasedVM](/fr/doc/glossary/#templatebasedvm).
You can also choose to use different [DisposableVM Templates](/fr/doc/glossary/#disposablevm-template) for different DisposableVMs.
...
If you wish to use a [Minimal TemplateVM](/fr/doc/templates/minimal/) as a DisposableVM Template, please see the [Minimal TemplateVM](/fr/doc/templates/minimal/) page.
...
**Note** You can use `qvm-pci` to [determine](/fr/doc/pci-devices/#qvm-pci-usage) the `<BDF>`.
Also, you will often need to include the `-o no-strict-reset=True` [option](/fr/doc/pci-devices/#no-strict-reset) with USB controllers.
...
6. Attach the network PCI device(s) to `disp-sys-net` (finding and assigning PCI devices can be found [here](/fr/doc/pci-devices/):
...
5. Attach the USB controller to the `disp-sys-usb`:
   >_**Note:**_ Most of the commonly used USB controllers (all Intel integrated controllers) require the `-o no-strict-reset=True` option to be set. Instructions detailing how this option is set can be found [here](/fr/doc/pci-devices/#no-strict-reset).
