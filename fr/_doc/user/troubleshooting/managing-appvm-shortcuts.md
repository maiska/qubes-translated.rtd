...
lang: fr
layout: doc
permalink: /fr/doc/managing-appvm-shortcuts/
redirect_from:
- /fr/doc/ManagingAppVmShortcuts/
- /fr/wiki/ManagingAppVmShortcuts/
ref: 202
...
2. Create a custom `.desktop` file in `~/.local/share/applications`.
   ...
   You may use `qvm-run` inside the `.desktop` file; see [Behind the scenes](/fr/doc/managing-appvm-shortcuts/#behind-the-scenes) for more details.
...
[See here](/fr/doc/disposablevm-customization/).
...
`qvm-sync-appmenus` works by invoking *GetAppMenus* [Qubes service](/fr/doc/qrexec/) in the target domain.
