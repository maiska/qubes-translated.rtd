...
In [How to Get Started](/fr/doc/how-to-get-started/), we covered the distinction
in Qubes OS between where you *install* your software and where you *run* your
software. Your software is installed in [templates](/fr/doc/glossary/#template).
Each template shares its root filesystem (i.e., all of its programs and system
files) with all the qubes based on it. [App qubes](/fr/doc/glossary/#app-qube) are
...
* [Fedora](/fr/doc/templates/fedora/) (default)
...
* When a template version you're using reaches
  [end-of-life](/fr/doc/supported-versions/).
...
[minimal](/fr/doc/templates/minimal/) version of the template, if one exists, by
...
[here](/fr/faq/#when-i-try-to-install-a-template-it-says-no-match-is-found).
...
Please see [How to Update](/fr/doc/how-to-update/).
...
[VM Troubleshooting](/fr/doc/vm-troubleshooting/).
...
Please see [How to Reinstall a Template](/fr/doc/reinstall-template/).
...
4. Base the [disposable template](/fr/doc/glossary/#disposable-template) on the new
   template.
...
|-------------------------------------------------|-----------------------------------------------------------|---------------------------------------------------------|
| [template](/fr/doc/glossary/#template)             | N/A (templates cannot be based on templates)              | everything                                              |
| [app qube](/fr/doc/glossary/#app-qube)<sup>3</sup> | `/etc/skel` to `/home`; `/usr/local.orig` to `/usr/local` | `/rw` (includes `/home`, `/usr/local`, and `bind-dirs`) |
| [disposable](/fr/doc/glossary/#disposable)         | `/rw` (includes `/home`, `/usr/local`, and `bind-dirs`)   | nothing                                                 |

<sup>1</sup>Upon creation  
<sup>2</sup>Following shutdown  
<sup>3</sup>Includes [disposable templates](/fr/doc/glossary/#disposable-template)
...
* Only install packages from trusted sources -- e.g. from the pre-configured
  ...
  enforced by default (at the [firewall qube level](/fr/doc/firewall/)), by not
  allowing any networking connectivity in the default template, except for
  access to the Fedora repos.

* Use [standalones](/fr/doc/glossary/#standalone) (see below) for installation of
...
by the [firewall qube](/fr/doc/firewall/), and again they should not fear this
...
* Some templates are available in ready-to-use binary form, but some of them
  ...
  Builder](/fr/doc/qubes-builder/). In particular, some template "flavors" are
  available in source code form only. For the technical details of the template
  system, please see [Template Implementation](/fr/doc/template-implementation/).
  Take a look at the [Qubes Builder](/fr/doc/qubes-builder/) documentation for
