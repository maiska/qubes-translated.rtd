...
lang: fr
layout: doc
permalink: /fr/doc/disposable-customization/
redirect_from:
- /fr/doc/DispVMCustomization/
- /fr/doc/UserDoc/DispVMCustomization/
- /fr/doc/disposablevm-customization/
- /fr/doc/dispvm-customization/
- /fr/wiki/UserDoc/DispVMCustomization/
ref: 174
...
A [disposable](/fr/doc/disposable/) can be based on any [app qube](/fr/doc/glossary/#app-qube).
You can also choose to use different [disposable templates](/fr/doc/glossary/#disposable-template) for different disposables.
...
The above default is used whenever a qube request starting a new disposable and do not specify which one (for example `qvm-open-in-dvm` tool). This can be also set in qube settings and will affect service calls from that qube. See [qrexec documentation](/fr/doc/qrexec/#specifying-vms-tags-types-targets-etc) for details.

If you wish to use a [Minimal Template](/fr/doc/templates/minimal/) as a disposable template, please see the [Minimal Template](/fr/doc/templates/minimal/) page.
...
You can use a [named disposable](/fr/doc/glossary/#named-disposable) for `sys-*` as long as it is stateless.
...
**Note** You can use `qvm-pci` to [determine](/fr/doc/how-to-use-pci-devices/#qvm-pci-usage) the `<BDF>`.
Also, you will often need to include the `-o no-strict-reset=True` [option](/fr/doc/how-to-use-pci-devices/#no-strict-reset) with USB controllers.
...
2. Set `disp-sys-net` virtualization mode to [hvm](/fr/doc/hvm/):
...
6. Attach the network PCI device(s) to `disp-sys-net` (finding and assigning PCI devices can be found [here](/fr/doc/how-to-use-pci-devices/):
...
5. Attach the USB controller to the `disp-sys-usb`:
   >_**Note:**_ Most of the commonly used USB controllers (all Intel integrated controllers) require the `-o no-strict-reset=True` option to be set. Instructions detailing how this option is set can be found [here](/fr/doc/how-to-use-pci-devices/#no-strict-reset).
...
8. Users should now follow instructions on [How to hide USB controllers from dom0](/fr/doc/usb-qubes/#how-to-hide-all-usb-controllers-from-dom0).
...
If the `disp-sys-usb` does not start, it could be due to a PCI passthrough problem. For more details on this issue along with possible solutions, users can look [here](/fr/doc/pci-troubleshooting/#pci-passthrough-issues).
