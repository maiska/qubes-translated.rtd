...
lang: fr
layout: doc
permalink: /fr/doc/backup-restore/
redirect_from:
- /fr/wiki/BackupRestore/
- /fr/doc/BackupRestore/
ref: 199
...
3. Select the destination for the backup:
   ...
   If you wish to send your backup to a [USB mass storage device](/fr/doc/usb/), you can use the directory selection widget to mount a connected device (under "Other locations" item on the left); or first mount the device in a VM, then select the mount point inside that VM as the backup destination.
...
2. Select the source location of the backup to be restored:
   ...
   - If your backup is located on a [USB mass storage device](/fr/doc/usb/), attach it first to another VM or select `sys-usb` in the next item.
...
- [Qubes R4 or newer](/fr/doc/backup-emergency-restore-v4/)
- [Qubes R3](/fr/doc/backup-emergency-restore-v3/)
- [Qubes R2 or older](/fr/doc/backup-emergency-restore-v2/)

Migrating between two physical machines
---------------------------------------
<a id="migrating-between-two-physical-machines"></a>

In order to migrate your Qubes system from one physical machine to another, simply follow the backup procedure on the old machine, [install Qubes](/fr/downloads/) on the new machine, and follow the restoration procedure on the new machine.
