...
lang: fr
layout: doc
permalink: /fr/doc/standalone-and-hvm/
redirect_from:
- /fr/wiki/HvmCreate/
- /fr/doc/hvm/
- /fr/doc/hvm-create/
...
<a id="standalonevms-and-hvms"></a>

A [StandaloneVM](/fr/doc/glossary/#standalonevm) is a type of VM in Qubes that is created by cloning a [TemplateVM](/fr/doc/templates/).
...
Meanwhile, a [Hardware-assisted Virtual Machine (HVM)](/fr/doc/glossary/#hvm), also known as a "Fully-Virtualized Virtual Machine," utilizes the virtualization extensions of the host CPU.
These are typically contrasted with [Paravirtualized (PV)](/fr/doc/glossary/#pv) VMs.
...
By default, every Qubes VM runs in [PVH](/fr/doc/glossary/#pvhvm) mode (which has security advantages over both PV and HVM) except for those with attached PCI devices, which run in HVM mode.
...
In fact, this is so common that [StandaloneHVMs](/fr/doc/glossary/#standalonehvm) are typically just called "HVMs."
Hence, this page covers both topics.

## Creating a StandaloneVM
<a id="creating-a-standalonevm"></a>
...
Normally Qubes agent scripts (or services on Windows) running within each AppVM are responsible for setting up networking within the VM according to the configuration created by Qubes (through [keys](/fr/doc/vm-interface/#qubesdb) exposed by dom0 to the VM).
...
There is [opt-in support](/fr/doc/networking/#ipv6) for IPv6 forwarding.

## Using TemplateBasedHVMs
<a id="using-templatebasedhvms"></a>

Qubes allows HVMs to share a common root filesystem from a select TemplateVM (see [TemplateHVM](/fr/doc/glossary/#templatehvm) and [TemplateBasedHVM](/fr/doc/glossary/#templatebasedhvm)).
...
Set memory as appropriate, and install the OS into this template in the same way you would install it into a normal HVM -- please see instructions on [this page](/fr/doc/hvm-create/).
...
Please see [this page](/fr/doc/windows-appvms/) for specific advice on installing and using Windows-based Templates.
...
HVM domains (including Windows VMs) can be [assigned PCI devices](/fr/doc/assigning-devices/) just like normal AppVMs.
...
- [Windows VMs](/fr/doc/windows-vm/)
- [LinuxHVMTips](/fr/doc/linux-hvm-tips/)